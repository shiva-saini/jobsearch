...
-----> action methods we are using to access the front end data and send to backend and performing operations accordingly.

-----> lader is similar to action it gives data before the component renders

-----> in the outlet context we can provide any value and use it in later pages .
...
--------in react errors bubbles up so try to avoid nesting things



-----> when there is an error it will first goes to server.js file from there it will go to errorHandler file and from there it will send the status code and status message.


//we will create middleware for this to reduce our code length
...
app.post(
  ...
  [[
    body("name")
      ...
      .isLength({ min: 50 })
      .withMessage("name must contains 50 chars"),
  ],
  (req, res, next) => {
